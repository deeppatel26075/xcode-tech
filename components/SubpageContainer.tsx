...
export default function SubpageContainer({ children }: SubpageContainerProps) {
  return (
    <div className="min-h-screen pt-28 pb-16 sm:py-24 px-4 sm:px-6 bg-sky-portal flex flex-col justify-start relative overflow-hidden">
      {/* Moving Sky Clouds */}
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-6 z-10">
        <FadeIn direction="none" delay={0.1} duration={0.6}>
          {/* Giant Main Transparent Card (Frosted Layered Glass Container) */}
          <div className="rounded-[2rem] sm:rounded-[2.2rem] border border-white/25 bg-white/[0.08] backdrop-blur-[24px] p-5 sm:p-8 lg:p-10 shadow-2xl relative overflow-hidden flex flex-col min-h-[80vh] w-full">
